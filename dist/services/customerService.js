...
exports.customerService = {
    ...
    async create(userData) {
        ...
        const { data: existingCustomer } = await database_1.supabase
            .from('customers')
            .select('id')
            .eq('phone', phone)
            .single();
    ...
    async update(id, updateData) {
        ...
        const { data: currentCustomer } = await database_1.supabase
            .from('customers')
        ...
        if (address_type !== undefined)
            customerUpdate.address_type = address_type;
        const { data: customer, error: customerError } = await database_1.supabase
            .from('customers')
      ...
      `)
        ...
        if (name) {
            ...
            const { data: refreshedCustomer } = await database_1.supabase
                .from('customers')
                .select(`
          *,
          profiles(email, role, name)
    ...
    async delete(id) {
        ...
        const { data: customerProducts } = await database_1.supabase
        ...
        const { data: customer } = await database_1.supabase
            .from('customers')
            .select('profile_id')
            .eq('id', id)
            .single();
        ...
        const { error: customerError } = await database_1.supabase
            .from('customers')
            .delete()
            .eq('id', id);
        if (customerError) {
